...
Articles.prototype.readAll = (req, res, data, cb) => {
    ...
    cb(null, {
        titles: pagginedArticles, meta: meta
    });
};
...
Articles.prototype.read = (req, res, data, cb) => {
...
Articles.prototype.create = (req, res, data, cb) => {
    if (valid.isValidArticle(data)) {
        data.id = uniqid();
        data.comments = [];
        data.date = Date.now() / 1000;
...
Articles.prototype.update = (req, res, data, cb) => {
...
Articles.prototype.delete = (req, res, data, cb) => {
    if (valid.isValidId(data)) {
